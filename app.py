...
# Route 4: Chức năng tải file hồ sơ (ĐÃ QUAY VỀ BẢN CŨ TẢI TXT)
...
# Route 5: Xóa hồ sơ bệnh án (CÓ MẬT KHẨU LÀ 1234)
@app.route('/delete_record/<int:record_id>', methods=['POST'])
def delete_record(record_id):
    # Mật khẩu cứng - KHÔNG AN TOÀN TRONG THỰC TẾ!
    correct_password = "1234" # <-- Mật khẩu đã được đặt là 1234
    
    # Lấy mật khẩu từ form
    submitted_password = request.form.get('password')

    if submitted_password != correct_password:
        # Nếu mật khẩu không đúng, trả về lỗi hoặc chuyển hướng lại
        return "Mật khẩu không đúng. Không thể xóa hồ sơ.", 403 # Lỗi 403 Forbidden

    record = db.get_or_404(Record, record_id) # Tìm bản ghi theo ID
    try:
        db.session.delete(record) # Xóa bản ghi khỏi phiên làm việc
        db.session.commit() # Lưu thay đổi vào database
        return redirect(url_for('history')) # Chuyển hướng về trang lịch sử sau khi xóa
    except Exception as e:
        db.session.rollback() # Hoàn tác nếu có lỗi
        print(f"Error deleting record: {e}") # In lỗi ra console để debug
        return "Có lỗi xảy ra khi xóa hồ sơ.", 500 # Trả về lỗi nếu không xóa được
