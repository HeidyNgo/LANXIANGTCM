...
def history():
    ...
    print("Số lượng hồ sơ tìm thấy:", len(all_records))  # <-- Dòng này giúp kiểm tra dữ liệu trong logs
...
def history():
    all_records = db.session.execute(db.select(Record).order_by(Record.created_at.desc())).scalars().all()
    print("Số lượng hồ sơ tìm thấy:", len(all_records))  # <--- dòng này giúp kiểm tra
    ...
