...
class Record(db.Model):
    ...
    created_at = db.Column(db.DateTime, nullable=False, default=lambda: datetime.now(pytz.timezone('Asia/Ho_Chi_Minh')))
...
def generate_report():
    ...
    consultation_date = datetime.now(pytz.timezone('Asia/Ho_Chi_Minh')).strftime("%B %d, %Y")
...
def history():
    ...
    query = db.select(Record).order_by(Record.created_at.desc())
    if search_name:
        query = query.filter(Record.patient_name.ilike(f"%{search_name}%"))
    all_records = db.session.execute(query).scalars().all()
...
def download_record(record_id):
    ...
    def draw_lines(title, content):
        nonlocal y
        lines = simpleSplit(content, font_name, 12, 480)
        if y <= 100:
            p.showPage()
            p.setFont(font_name, 12)
            y = 750
    ...
    draw_lines("LANXIANG Traditional Chinese Medicine", "")
    draw_lines("TCM Case Report", "")
    draw_lines(f"Patient Name: {record.patient_name}", "")
    draw_lines(f"Created At: {record.created_at.strftime('%d/%m/%Y')}", "")
    draw_lines("Symptoms:", record.symptoms)
    draw_lines("Treatment:", record.treatment)
    draw_lines("AI Report:", record.ai_report)
